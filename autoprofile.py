...
async def autobio_loop():
    ...
    while AUTOBIOSTART:
        ...
        HM = time.strftime("%H:%M")
        bio = f" دانلود ساعت مورد نظرت : {HM} "
        LOGS.info(bio)
        try:
...
async def _(event):
    ...
    await edit_delete(event, "`رئیس ، اسم تایمدار با موفقیت فعال شد😉`")
    await autoname_loop()
...
async def _(event):
    ...
    await edit_delete(event, "`رئیس ، بیو تایمدار با موفقیت فعال شد 😉`")
    await autobio_loop()
...
async def _(event):  # sourcery no-metrics
    ...
    if input_str == "autoname":
        if gvarstatus("autoname") is not None and gvarstatus("autoname") == "true":
            ...
            return await edit_delete(event, "`رئیس ، اسم تایمدار با موفقیت غیر فعال شد`")
